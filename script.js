		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			function render_gcode(){
				...
				var first_move = true;
				gcode_output+= "G0 Z"+g_penup+" F"+g_travel_speed+"\r\n";
				gcode_output+= "G0 X"+(GCODE_DATA[0][0]).toFixed(2)+" Y"+(GCODE_DATA[0][1]).toFixed(2)+" F"+g_travel_speed+"\r\n";
				gcode_output+= "G0 Z"+g_pendown+" F"+g_travel_speed+"\r\n";
				for(a=0; a<GCODE_DATA.length; a++){
					gcode_output += "G1 X"+(GCODE_DATA[a][2]).toFixed(2)+" Y"+(GCODE_DATA[a][3]).toFixed(2)+" F"+(first_move?g_travel_speed:g_draw_speed)+"\r\n";
					if(typeof GCODE_DATA[a+1]!='undefined' && (GCODE_DATA[a][2]!=GCODE_DATA[a+1][0] || GCODE_DATA[a][3]!=GCODE_DATA[a+1][1])){
						gcode_output+= "G0 Z"+g_penup+" F"+g_travel_speed+"\r\n";
						gcode_output+= "G0 X"+(GCODE_DATA[a+1][0]).toFixed(2)+" Y"+(GCODE_DATA[a+1][1]).toFixed(2)+" F"+g_travel_speed+"\r\n";
						gcode_output+= "G0 Z"+g_pendown+" F"+g_travel_speed+"\r\n";
					}
					first_move = false;
				}
